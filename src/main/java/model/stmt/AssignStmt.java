	 ...
	 public PrgState execute(PrgState state) throws VarNotDefined, DivByZero, CustomException {
		 ...
		 if(symTbl.isDefined(id)){
			 ...
			 if( typId.equals(new IntType()) && (val.getType()).equals(new StringType()) ) {
				 ...
				 symTbl.update(id, new IntValue(new_integer,val.getTaint(),val.getSecurityClass()));
			 }
				 ...
				 symTbl.update(id, new StringValue(new_string,val.getTaint(),val.getSecurityClass()));
			 }
